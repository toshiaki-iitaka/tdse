import numpy as np
import math
from scipy.special import jv # For Bessel function of the first kind
import matplotlib.pyplot as plt

# --- Parameters ---
PI = 3.14159265358979323 # Fortran's PI parameter
NMAX = 100  # Maximum number of rooms/points

# --- Subroutine CH: Implements Chebyshev propagation ---
# This subroutine is used by the main program and thus is not removed.
def ch_subroutine(ha_norm, hb_norm, egrid, emin, ph0, t, nmax_val):
    """
    Performs time evolution using the Chebyshev propagation method.

    Args:
        ha_norm (np.ndarray): Normalized diagonal Hamiltonian elements.
        hb_norm (np.ndarray): Normalized off-diagonal Hamiltonian elements.
        egrid (float): Energy grid span (EMAX - EMIN).
        emin (float): Minimum energy.
        ph0 (np.ndarray): Initial wave function (complex array).
        t (float): Total time for propagation.
        nmax_val (int): Maximum number of points (NMAX).

    Returns:
        np.ndarray: The propagated wave function (PH).
    """
    iu = 0.0 + 1.0j
    eps = 1e-16 # Convergence criterion for Bessel function series (from Fortran PARAMETER)
    imax = 1000 # Maximum iterations for Chebyshev series (from Fortran PARAMETER)

    # Initialize P, Q, R, PH for the Chebyshev series calculation
    # These are temporary arrays within the subroutine, distinct from main program's P,Q,R,S,T,U.
    p_cheb = np.zeros(nmax_val, dtype=np.complex128)
    q_cheb = np.zeros(nmax_val, dtype=np.complex128)
    r_cheb = np.zeros(nmax_val, dtype=np.complex128)
    ph_cheb = np.zeros(nmax_val, dtype=np.complex128) # Resulting propagated wave function

    # DO 20 N=1,NMAX
    # P(N)=PH0(N)
    # 20 CONTINUE
    for n_idx in range(nmax_val):
        p_cheb[n_idx] = ph0[n_idx]

    # Q(1) = -IU*(HA(1)*P(1)+HB(1)*P(2))
    q_cheb[0] = -iu * (ha_norm[0] * p_cheb[0] + hb_norm[0] * p_cheb[1])
    # DO 30 N=2,NMAX-1
    # Q(N) = -IU*(HB(N-1)*P(N-1)+HA(N)*P(N)+HB(N)*P(N+1))
    # 30 CONTINUE
    for n_idx in range(1, nmax_val - 1):
        q_cheb[n_idx] = -iu * (hb_norm[n_idx-1] * p_cheb[n_idx-1] + ha_norm[n_idx] * p_cheb[n_idx] + hb_norm[n_idx] * p_cheb[n_idx+1])
    # Q(NMAX)= -IU*(HB(NMAX-1)*P(NMAX-1)+HA(NMAX)*P(NMAX))
    q_cheb[nmax_val-1] = -iu * (hb_norm[nmax_val-2] * p_cheb[nmax_val-2] + ha_norm[nmax_val-1] * p_cheb[nmax_val-1])

    # Initial terms of the Chebyshev expansion
    # AN=DJBES(0,EGRID*T/2)
    an_0 = jv(0, egrid * t / 2.0) # Using scipy's Bessel function
    # DO 40 N=1,NMAX
    # PH(N)=AN*P(N)
    # 40 CONTINUE
    for n_idx in range(nmax_val):
        ph_cheb[n_idx] = an_0 * p_cheb[n_idx]

    # AN=2*DJBES(1,EGRID*T/2)
    an_1 = 2 * jv(1, egrid * t / 2.0) # Using scipy's Bessel function
    # DO 45 N=1,NMAX
    # PH(N)=PH(N)+AN*Q(N)
    # 45 CONTINUE
    for n_idx in range(nmax_val):
        ph_cheb[n_idx] = ph_cheb[n_idx] + an_1 * q_cheb[n_idx]

    # Main Chebyshev series loop
    # DO 1000 I=2, IMAX, 3
    # The Fortran GOTO logic for convergence is replaced by a Python `break`.
    for i in range(2, imax + 1, 3):
        # AN=2*DJBES(I,EGRID*T/2)
        an_i = 2 * jv(i, egrid * t / 2.0) # Using scipy's Bessel function
        # IF(ABS(AN).LT.EPS) GOTO 2000
        if abs(an_i) < eps:
            break # Exit loop if Bessel function term is negligible

        # R(1) = -2*IU*(HA(1)*Q(1)+HB(1)*Q(2))+P(1)
        r_cheb[0] = -2 * iu * (ha_norm[0] * q_cheb[0] + hb_norm[0] * q_cheb[1]) + p_cheb[0]
        # PH(1)=PH(1) +AN*R(1)
        ph_cheb[0] = ph_cheb[0] + an_i * r_cheb[0]
        # DO 70 N=2,NMAX-1
        # R(N)=-2*IU*(HB(N-1)*Q(N-1)+HA(N)*Q(N)+HB(N)*Q(N+1))+P(N)
        # PH(N)=PH(N)+AN*R(N)
        # 70 CONTINUE
        for n_idx in range(1, nmax_val - 1):
            r_cheb[n_idx] = -2 * iu * (hb_norm[n_idx-1] * q_cheb[n_idx-1] + ha_norm[n_idx] * q_cheb[n_idx] + hb_norm[n_idx] * q_cheb[n_idx+1]) + p_cheb[n_idx]
            ph_cheb[n_idx] = ph_cheb[n_idx] + an_i * r_cheb[n_idx]
        # R(NMAX)=-2*IU*(HB(NMAX-1)*Q(NMAX-1)+HA(NMAX)*Q(NMAX))+P(NMAX)
        r_cheb[nmax_val-1] = -2 * iu * (hb_norm[nmax_val-2] * q_cheb[nmax_val-2] + ha_norm[nmax_val-1] * q_cheb[nmax_val-1]) + p_cheb[nmax_val-1]
        # PH(NMAX)=PH(NMAX)+AN*R(NMAX)
        ph_cheb[nmax_val-1] = ph_cheb[nmax_val-1] + an_i * r_cheb[nmax_val-1]

        # AN=2*DJBES(I+1,EGRID*T/2)
        an_i_plus_1 = 2 * jv(i + 1, egrid * t / 2.0) # Using scipy's Bessel function
        if abs(an_i_plus_1) < eps:
            break

        # P(1) = -2*IU*(HA(1)*R(1)+HB(1)*R(2))+Q(1)
        p_temp_0 = -2 * iu * (ha_norm[0] * r_cheb[0] + hb_norm[0] * r_cheb[1]) + q_cheb[0]
        # PH(1)=PH(1) +AN*P(1)
        ph_cheb[0] = ph_cheb[0] + an_i_plus_1 * p_temp_0
        # DO 71 N=2,NMAX-1
        # P(N)=-2*IU*(HB(N-1)*R(N-1)+HA(N)*R(N)+HB(N)*R(N+1))+Q(N)
        # PH(N)=PH(N)+AN*P(N)
        # 71 CONTINUE
        for n_idx in range(1, nmax_val - 1):
            p_temp_n = -2 * iu * (hb_norm[n_idx-1] * r_cheb[n_idx-1] + ha_norm[n_idx] * r_cheb[n_idx] + hb_norm[n_idx] * r_cheb[n_idx+1]) + q_cheb[n_idx]
            ph_cheb[n_idx] = ph_cheb[n_idx] + an_i_plus_1 * p_temp_n
            p_cheb[n_idx] = p_temp_n # Update p_cheb for next iteration
        # P(NMAX)=-2*IU*(HB(NMAX-1)*R(NMAX-1)+HA(NMAX)*R(NMAX))+Q(NMAX)
        p_temp_nmax = -2 * iu * (hb_norm[nmax_val-2] * r_cheb[nmax_val-2] + ha_norm[nmax_val-1] * r_cheb[nmax_val-1]) + q_cheb[nmax_val-1]
        ph_cheb[nmax_val-1] = ph_cheb[nmax_val-1] + an_i_plus_1 * p_temp_nmax
        p_cheb[nmax_val-1] = p_temp_nmax # Update p_cheb for next iteration
        p_cheb[0] = p_temp_0 # Update p_cheb[0]

        # AN=2*DJBES(I+2,EGRID*T/2)
        an_i_plus_2 = 2 * jv(i + 2, egrid * t / 2.0) # Using scipy's Bessel function
        if abs(an_i_plus_2) < eps:
            break

        # Q(1) = -2*IU*(HA(1)*P(1)+HB(1)*P(2))+R(1)
        q_temp_0 = -2 * iu * (ha_norm[0] * p_cheb[0] + hb_norm[0] * p_cheb[1]) + r_cheb[0]
        # PH(1)=PH(1) +AN*Q(1)
        ph_cheb[0] = ph_cheb[0] + an_i_plus_2 * q_temp_0
        # DO 72 N=2,NMAX-1
        # Q(N)=-2*IU*(HB(N-1)*P(N-1)+HA(N)*P(N)+HB(N)*P(N+1))+R(N)
        # PH(N)=PH(N)+AN*Q(N)
        # 72 CONTINUE
        for n_idx in range(1, nmax_val - 1):
            q_temp_n = -2 * iu * (hb_norm[n_idx-1] * p_cheb[n_idx-1] + ha_norm[n_idx] * p_cheb[n_idx] + hb_norm[n_idx] * p_cheb[n_idx+1]) + r_cheb[n_idx]
            ph_cheb[n_idx] = ph_cheb[n_idx] + an_i_plus_2 * q_temp_n
            q_cheb[n_idx] = q_temp_n # Update q_cheb for next iteration
        # Q(NMAX)=-2*IU*(HB(NMAX-1)*P(NMAX-1)+HA(NMAX)*P(NMAX))+R(NMAX)
        q_temp_nmax = -2 * iu * (hb_norm[nmax_val-2] * p_cheb[nmax_val-2] + ha_norm[nmax_val-1] * p_cheb[nmax_val-1]) + r_cheb[nmax_val-1]
        ph_cheb[nmax_val-1] = ph_cheb[nmax_val-1] + an_i_plus_2 * q_temp_nmax
        q_cheb[nmax_val-1] = q_temp_nmax # Update q_cheb for next iteration
        q_cheb[0] = q_temp_0 # Update q_cheb[0]

    else: # This block executes if the loop completes without a 'break'
        print('CHEBYSHEV NOT CONVERGE')

    # Final transformation
    # DO 80 N=1,NMAX
    # PH(N)=PH(N)*EXP(-IU*(EGRID/2+EMIN)*T)
    # 80 CONTINUE
    for n_idx in range(nmax_val):
        ph_cheb[n_idx] = ph_cheb[n_idx] * np.exp(-iu * (egrid / 2.0 + emin) * t)

    return ph_cheb

# --- Main Program: NROOM ---
def nroom_program():
    """
    Main program to simulate N-room quantum mechanics using a 5-step time evolution method.
    It compares the 5-step method's result with an exact solution obtained via Chebyshev propagation.
    """
    # Define complex imaginary unit
    iu = 0.0 + 1.0j

    # --- Initial Data ---
    # OPEN(20,FILE='sc4.dat')
    try:
        f_sc4_out = open('sc4.dat', 'w')
    except Exception as e:
        print(f"Error opening sc4.dat: {e}")
        return
    
    plot_y = [[] for _ in range(6) ]

    # WRITE(*,*) 'ALPHA'
    # READ(*,*) ALPHA
    print('ALPHA')
    try:
        alpha = float(input())
    except ValueError:
        print("Invalid input. Please enter a number for ALPHA.")
        f_sc4_out.close()
        return

    dx = 1.0
    dx2 = dx * dx
    
    # EMAX=1/DX2+1
    emax_initial = 1.0 / dx2 + 1.0
    
    # DT=ALPHA/EMAX
    dt = alpha / emax_initial
    
    # MTIME=(2*PI/ALPHA) * 40
    # MTIME=MIN(MTIME,100000)
    mtime = (2 * PI / alpha) * 40
    mtime = min(int(mtime), 100000) # Ensure MTIME is an integer

    # --- Calculation of Hamiltonian ---
    # DIMENSION HA(NMAX),HB(NMAX-1),V(NMAX)
    ha = np.zeros(NMAX, dtype=np.float64)
    hb = np.zeros(NMAX - 1, dtype=np.float64) # HB has NMAX-1 elements
    v_potential = np.zeros(NMAX, dtype=np.float64)

    # DO 50 N=1,NMAX
    # IF(45.LE.N .AND. N.LE.55) THEN
    #   V(N)=0.05*COS((I-50)/5.0 * PI/2)**2  <- Assuming 'I' should be 'N'
    # ELSE
    #   V(N)=0
    # ENDIF
    # HA(N) = V(N)
    # 50 CONTINUE
    for n_idx in range(NMAX):
        # Fortran N is (n_idx + 1) in Python
        if 45 <= (n_idx + 1) <= 55:
            # (I-50) in Fortran is ((n_idx+1)-50) in Python
            v_potential[n_idx] = 0.05 * (np.cos(((n_idx + 1) - 50) / 5.0 * PI / 2.0))**2
        else:
            v_potential[n_idx] = 0.0
        ha[n_idx] = v_potential[n_idx] # HA(N) = V(N)

    # DO 51 N=1,NMAX-1
    # HB(N) = -0.5/DX2
    # 51 CONTINUE
    for n_idx in range(NMAX - 1):
        hb[n_idx] = -0.5 / dx2

    # --- Normalization of Hamiltonian (for Chebyshev subroutine) ---
    # VMAX=0
    # VMIN=0
    # DO 10 N=1,NMAX
    # IF(V(N).GT.VMAX) VMAX=V(N)
    # IF(V(N).LT.VMIN) VMIN=V(N)
    # 10 CONTINUE
    # The Fortran code then hardcodes VMAX=1.0, overriding the calculation.
    # So, we just use the hardcoded value directly.
    vmax_actual = 1.0
    print(f'VMAX={vmax_actual:15.7E}')

    # EMAX=1/DX2 +VMAX
    # EMIN=-1/DX2
    emax_cheb = 1.0 / dx2 + vmax_actual
    emin_cheb = -1.0 / dx2
    
    # EGRID=EMAX-EMIN
    egrid = emax_cheb - emin_cheb

    # DIMENSION HAN(NMAX),HBN(NMAX-1)
    han_norm = np.zeros(NMAX, dtype=np.float64)
    hbn_norm = np.zeros(NMAX - 1, dtype=np.float64)

    # DO 20 N=1,NMAX
    # HAN(N)=(HA(N)-EGRID/2-EMIN)*2/EGRID
    # IF(N.LT.NMAX) HBN(N)=HB(N)*2/EGRID
    # 20 CONTINUE
    for n_idx in range(NMAX):
        han_norm[n_idx] = (ha[n_idx] - egrid / 2.0 - emin_cheb) * 2.0 / egrid
        if n_idx < NMAX - 1:
            hbn_norm[n_idx] = hb[n_idx] * 2.0 / egrid

    print(f'EGRID={egrid:15.7E}')
    print('TIME,M') # 'M' is just a label in Fortran output, not a variable

    # --- Setting Initial Wave Function (Gaussian Wave Packet) ---
    # COMPLEX*16 PH0(NMAX)
    ph0 = np.zeros(NMAX, dtype=np.complex128)
    
    # X0=0.25*DX*NMAX
    # SG=0.1*DX*NMAX
    # PA=0.1*2*PI/DX
    x0 = 0.25 * dx * NMAX
    sg = 0.1 * dx * NMAX
    pa_momentum = 0.1 * 2 * PI / dx # Renamed to avoid conflict with normalization PA

    # DO 60 N=1,NMAX
    # X=N*DX
    # PH0(N)= EXP(-0.5*(X-X0)**2/SG**2) * EXP(IU*PA*(X-X0))
    # 60 CONTINUE
    for n_idx in range(NMAX):
        x = (n_idx + 1) * dx # Fortran N*DX
        ph0[n_idx] = np.exp(-0.5 * (x - x0)**2 / sg**2) * np.exp(iu * pa_momentum * (x - x0))

    # --- Normalization of Initial Wave Function ---
    pa_norm_factor = 0.0
    # DO 101 N=1,NMAX
    # PA = PA + ABS(PH0(N))**2
    # 101 CONTINUE
    for n_idx in range(NMAX):
        pa_norm_factor += np.abs(ph0[n_idx])**2
    pa_norm_factor = np.sqrt(pa_norm_factor)

    # COMPLEX*16 P(NMAX),Q(NMAX),R(NMAX),S(NMAX),T(NMAX),U(NMAX)
    p_psi = np.zeros(NMAX, dtype=np.complex128)
    q_psi = np.zeros(NMAX, dtype=np.complex128)
    r_psi = np.zeros(NMAX, dtype=np.complex128)
    s_psi = np.zeros(NMAX, dtype=np.complex128)
    t_psi = np.zeros(NMAX, dtype=np.complex128)
    u_temp = np.zeros(NMAX, dtype=np.complex128) # U is a temporary array in the main loop

    # DO 110 N=1,NMAX
    # PH0(N)=PH0(N)/PA
    # P(N)=PH0(N)
    # 110 CONTINUE
    for n_idx in range(NMAX):
        ph0[n_idx] = ph0[n_idx] / pa_norm_factor
        p_psi[n_idx] = ph0[n_idx] # P is psi(t=0)

    # Initial calls to CH for the first few time steps (psi(dt), psi(2dt), psi(3dt))
    # CALL CH(HAN,HBN,EGRID,EMIN,PH0,DT,Q)
    q_psi[:] = ch_subroutine(han_norm, hbn_norm, egrid, emin_cheb, ph0, dt, NMAX)
    # CALL CH(HAN,HBN,EGRID,EMIN,PH0,2*DT,R)
    r_psi[:] = ch_subroutine(han_norm, hbn_norm, egrid, emin_cheb, ph0, 2 * dt, NMAX)
    # CALL CH(HAN,HBN,EGRID,EMIN,PH0,3*DT,S)
    s_psi[:] = ch_subroutine(han_norm, hbn_norm, egrid, emin_cheb, ph0, 3 * dt, NMAX)
    # T is not explicitly initialized here in Fortran, but will be calculated in the loop.
    # For clarity, we'll ensure it's a valid numpy array.
    t_psi[:] = np.zeros(NMAX, dtype=np.complex128)


    # --- Time Evolution Loop (5-step method) ---
    # DO 1000 ITIME=3,MTIME,5
    for itime in range(3, mtime + 1, 5): # Loop starts at 3, increments by 5
        tt = dt * itime # Current simulation time

        # --- Output Wave Function ---
        # IF(MOD(ITIME,30).EQ. 3) THEN
        if (itime % 30) == 3:
            # CALL CH(HAN,HBN,EGRID,EMIN,PH0,TT,PH)
            # PH is the exact solution at time TT, calculated by Chebyshev
            ph_exact = ch_subroutine(han_norm, hbn_norm, egrid, emin_cheb, ph0, tt, NMAX)

            anorm = 0.0
            ovr = 0.0 + 0.0j
            # DO 150 N=1,NMAX
            # ANORM=ANORM+ABS(S(N))**2
            # OVR=OVR+DCONJG(S(N))*PH(N)
            # 150 CONTINUE
            for n_idx in range(NMAX):
                anorm += np.abs(s_psi[n_idx])**2 # S is the current state from the 5-step method for output
                ovr += np.conjugate(s_psi[n_idx]) * ph_exact[n_idx]

            # WRITE(20,100) ITIME,EMAX*TT,ABS(ANORM-1),
            # &ABS(1- ABS(OVR)),ABS(DATAN2(DIMAG(OVR),DBLE(OVR)))
            # 100 FORMAT(1H ,I7,30E15.7)
            file_output = (
                f"{itime:7d} {emax_initial*tt:15.7E} {np.abs(anorm-1):15.7E} "
                f"{np.abs(1.0 - np.abs(ovr)):15.7E} {np.abs(math.atan2(ovr.imag, ovr.real)):15.7E}"
            )
            print(file_output) # Print to console as well for debugging/monitoring
            f_sc4_out.write(file_output + "\n")

            plot_y[0].append(itime)
            plot_y[1].append(emax_initial*tt)
            plot_y[2].append(abs(anorm-1))
            plot_y[3].append(abs(1.0 - abs(ovr)))
            plot_y[4].append(abs(abs(np.arctan2(ovr.imag, ovr.real))))
            #plot_y[5].append(abs(em*dt)**3 / 6.0 * itime)

        # --- Time Evolution (5-step method) ---
        # These are in-place updates. The variables P, Q, R, S, T
        # are updated sequentially, with each step using the most recently
        # updated values of the variables.

        # DO 80 N=1,NMAX
        # U(N) = (2*Q(N)-R(N)+2*S(N))/3.0
        # 80 CONTINUE
        u_temp[:] = (2 * q_psi - r_psi + 2 * s_psi) / 3.0
        
        # T(1)= -4*IU*DT*(HA(1)*U(1)+HB(1)*U(2)) + P(1)
        t_psi[0] = -4 * iu * dt * (ha[0] * u_temp[0] + hb[0] * u_temp[1]) + p_psi[0]
        # DO 70 N=2,NMAX-1
        # T(N)= -4*IU*DT*(HB(N-1)*U(N-1)+HA(N)*U(N)+HB(N)*U(N+1)) + P(N)
        # 70 CONTINUE
        for n_idx in range(1, NMAX - 1):
            t_psi[n_idx] = -4 * iu * dt * (hb[n_idx-1] * u_temp[n_idx-1] + ha[n_idx] * u_temp[n_idx] + hb[n_idx] * u_temp[n_idx+1]) + p_psi[n_idx]
        # T(NMAX)=-4*IU*DT*(HB(NMAX-1)*U(NMAX-1)+HA(NMAX)*U(NMAX)) + P(NMAX)
        t_psi[NMAX-1] = -4 * iu * dt * (hb[NMAX-2] * u_temp[NMAX-2] + ha[NMAX-1] * u_temp[NMAX-1]) + p_psi[NMAX-1]

        # DO 81 N=1,NMAX
        # U(N) = (2*R(N)-S(N)+2*T(N))/3.0
        # 81 CONTINUE
        # Uses current R, S, and the NEW T
        u_temp[:] = (2 * r_psi - s_psi + 2 * t_psi) / 3.0
        
        # P(1) = -4*IU*DT*(HA(1)*U(1)+HB(1)*U(2)) +Q(1)
        p_psi[0] = -4 * iu * dt * (ha[0] * u_temp[0] + hb[0] * u_temp[1]) + q_psi[0]
        # DO 71 N=2,NMAX-1
        # P(N)= -4*IU*DT*(HB(N-1)*U(N-1)+HA(N)*U(N)+HB(N)*U(N+1))+Q(N)
        # 71 CONTINUE
        for n_idx in range(1, NMAX - 1):
            p_psi[n_idx] = -4 * iu * dt * (hb[n_idx-1] * u_temp[n_idx-1] + ha[n_idx] * u_temp[n_idx] + hb[n_idx] * u_temp[n_idx+1]) + q_psi[n_idx]
        # P(NMAX)=-4*IU*DT*(HB(NMAX-1)*U(NMAX-1)+HA(NMAX)*U(NMAX)) + Q(NMAX)
        p_psi[NMAX-1] = -4 * iu * dt * (hb[NMAX-2] * u_temp[NMAX-2] + ha[NMAX-1] * u_temp[NMAX-1]) + q_psi[NMAX-1]

        # DO 82 N=1,NMAX
        # U(N) = (2*S(N)-T(N)+2*P(N))/3.0
        # 82 CONTINUE
        # Uses current S, and the NEW T, P
        u_temp[:] = (2 * s_psi - t_psi + 2 * p_psi) / 3.0
        
        # Q(1) = -4*IU*DT*(HA(1)*U(1)+HB(1)*U(2)) + R(1)
        q_psi[0] = -4 * iu * dt * (ha[0] * u_temp[0] + hb[0] * u_temp[1]) + r_psi[0]
        # DO 72 N=2,NMAX-1
        # Q(N)= -4*IU*DT*(HB(N-1)*U(N-1)+HA(N)*U(N)+HB(N)*U(N+1))+R(N)
        # 72 CONTINUE
        for n_idx in range(1, NMAX - 1):
            q_psi[n_idx] = -4 * iu * dt * (hb[n_idx-1] * u_temp[n_idx-1] + ha[n_idx] * u_temp[n_idx] + hb[n_idx] * u_temp[n_idx+1]) + r_psi[n_idx]
        # Q(NMAX)=-4*IU*DT*(HB(NMAX-1)*U(NMAX-1)+HA(NMAX)*U(NMAX))+R(NMAX)
        q_psi[NMAX-1] = -4 * iu * dt * (hb[NMAX-2] * u_temp[NMAX-2] + ha[NMAX-1] * u_temp[NMAX-1]) + r_psi[NMAX-1]

        # DO 83 N=1,NMAX
        # U(N) = (2*T(N)-P(N)+2*Q(N))/3.0
        # 83 CONTINUE
        # Uses the NEW T, P, Q
        u_temp[:] = (2 * t_psi - p_psi + 2 * q_psi) / 3.0
        
        # R(1) = -4*IU*DT*(HA(1)*U(1)+HB(1)*U(2))+S(1)
        r_psi[0] = -4 * iu * dt * (ha[0] * u_temp[0] + hb[0] * u_temp[1]) + s_psi[0]
        # DO 73 N=2,NMAX-1
        # R(N)= -4*IU*DT*(HB(N-1)*U(N-1)+HA(N)*U(N)+HB(N)*U(N+1))+S(N)
        # 73 CONTINUE
        for n_idx in range(1, NMAX - 1):
            r_psi[n_idx] = -4 * iu * dt * (hb[n_idx-1] * u_temp[n_idx-1] + ha[n_idx] * u_temp[n_idx] + hb[n_idx] * u_temp[n_idx+1]) + s_psi[n_idx]
        # R(NMAX)=-4*IU*DT*(HB(NMAX-1)*U(NMAX-1)+HA(NMAX)*U(NMAX))+S(NMAX)
        r_psi[NMAX-1] = -4 * iu * dt * (hb[NMAX-2] * u_temp[NMAX-2] + ha[NMAX-1] * u_temp[NMAX-1]) + s_psi[NMAX-1]

        # DO 84 N=1,NMAX
        # U(N) = (2*P(N)-Q(N)+2*R(N))/3.0
        # 84 CONTINUE
        # Uses the NEW P, Q, R
        u_temp[:] = (2 * p_psi - q_psi + 2 * r_psi) / 3.0
        
        # S(1) = -4*IU*DT*(HA(1)*U(1)+HB(1)*U(2))+T(1)
        s_psi[0] = -4 * iu * dt * (ha[0] * u_temp[0] + hb[0] * u_temp[1]) + t_psi[0]
        # DO 74 N=2,NMAX-1
        # S(N)= -4*IU*DT*(HB(N-1)*U(N-1)+HA(N)*U(N)+HB(N)*U(N+1))+T(N)
        # 74 CONTINUE
        for n_idx in range(1, NMAX - 1):
            s_psi[n_idx] = -4 * iu * dt * (hb[n_idx-1] * u_temp[n_idx-1] + ha[n_idx] * u_temp[n_idx] + hb[n_idx] * u_temp[n_idx+1]) + t_psi[n_idx]
        # S(NMAX)=-4*IU*DT*(HB(NMAX-1)*U(NMAX-1)+HA(NMAX)*U(NMAX))+T(NMAX)
        s_psi[NMAX-1] = -4 * iu * dt * (hb[NMAX-2] * u_temp[NMAX-2] + ha[NMAX-1] * u_temp[NMAX-1]) + t_psi[NMAX-1]


    #
    # plot
    #
    plt.gca().clear()
    plt.title('SC4')
    plt.xlabel(r'Emax*t')
    plt.ylabel(r'error')
    plt.xscale('log')
    plt.yscale('log')
    plt.xlim([1,100])
    plt.ylim([1e-20,1])
    plt.plot(plot_y[1],plot_y[2], marker='o',    linestyle='None',  label='ε_norm')
    plt.plot(plot_y[1],plot_y[4], marker='o',    linestyle='None',  label='ε_phase')
    #plt.plot(plot_y[1],plot_y[5], marker='None', linestyle='dashed',label='ε_theory')
    plt.legend()
    plt.show()
    plt.savefig("sc4.png")

    # Close the output file
    f_sc4_out.close()
    print("Program finished. Results written to sc4.dat")

# Call the main program function
if __name__ == '__main__':
    nroom_program()
    
